...
def test_scl(fasterRCNN, output_dir, dataset, cfg, class_agnostic, imdb, dataset_name, roidb=None):
    ...
    gt_features = [ [] for i in range(num_classes-1)]
    ...
    with torch.no_grad():
        for i in range(num_images):
            ...
            gt_rois = torch.cat([torch.zeros(gt_boxes.size(0)).cuda().view(-1,1), gt_boxes[:,:4]], 1)
            gt_feat = fasterRCNN.RCNN_roi_align(base_feat, gt_rois.view(-1, 5))
            gt_feat = fasterRCNN._head_to_tail(gt_feat)

            for j in range(1, num_classes):
                inds = (gt_boxes[:,-1]==j).nonzero().view(-1)
                if inds.size(0) != 0:
                    gt_features[j-1].append(gt_feat[inds].view(-1,feat_dim).detach())

    det_file = os.path.join(output_dir,  'gt_feats.pkl')
    print(det_file)
